...
export function HeroCanvasScroll() {
  ...
  // Apple Design Kinetic Split Name Transforms
  // "MAHABOOB" slides left, "SUHAIL" slides right, both fade out as frames play
  const mahaboobX = useTransform(scrollYProgress, [0, 0.65], ["0%", "-80%"])
  const suhailX = useTransform(scrollYProgress, [0, 0.65], ["0%", "80%"])
  ...
  // Handle frame drawing on scroll progress change (Maps scroll progress to 100 frames cleanly)
  useMotionValueEvent(scrollYProgress, "change", (latest) => {
    // Map scroll 0 -> 0.7 to frames 0 -> 99. Frame 100 stays pinned on canvas while name completes fading
    const progressFactor = Math.min(1, latest / 0.7)
  ...
  return (
    <div ref={containerRef} className="relative h-[165vh] bg-[#09090b]">
